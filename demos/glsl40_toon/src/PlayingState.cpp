...
void PlayingState::OnStart()
{
	...
	projectionMatrix = glm::perspective(glm::radians(60.0f), (float)windowWidth/(float)windowHeight, 0.1f, 300.0f);
	worldLight = new WorldLight(vec3(1, 1, 1), vec3(100, 200, 100));
	...
	entities.push_back(new PlayerCube(vec3(133, 200, 127), vec3(0, 0, 0), vec3(1, 1, 1)));
	entities.back()->Initialize();
	terrain->AddChild(entities.back());
...
{
	TerrainGenerator terrainGenerator = TerrainGenerator(8, vector<unsigned char>{128, 128, 128, 128}, 512, 1.0f, 42);
	...
	int dimensions = (int)pow(2, 8) + 1;
	terrain = new Terrain(dimensions, dimensions, 4, camera, heightMapPath, normalMapPath);
